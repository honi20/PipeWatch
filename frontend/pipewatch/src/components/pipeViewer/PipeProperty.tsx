import React, { useEffect, useState, ChangeEvent } from "react";
import ChevronRightIcon from "@mui/icons-material/ChevronRight";
import { PipeMaterialListbox } from "./listbox/PipeMaterialListbox";
import { Input, Button } from "@headlessui/react";
import clsx from "clsx";

interface PipeType {
  pipeName: string;
  pipeArea: string;
  pipeFloor: number;
  pipeMaterial: string;
  outerDiameter: number;
  innerDiameter: number;
  fluidMaterial: string;
  flowRate: number;
}
interface PipePropertyProps {
  pipe: PipeType;
  onViewChange: () => void;
}

export const PipeProperty: React.FC<PipePropertyProps> = ({
  pipe,
  onViewChange,
}) => {
  const [pipeMaterial, setPipeMaterial] = useState(pipe.pipeMaterial);
  const [outerDiameter, setOuterDiameter] = useState(pipe.outerDiameter || 0);
  const [innerDiameter, setInnerDiameter] = useState(pipe.innerDiameter || 0);
  const [flowRate, setFlowRate] = useState(pipe.flowRate || 0);

  // input이 바뀌었을때 true
  const [isChanged, setIsChanged] = useState(false);

  // 값이 변경될 때마다 isChanged 상태 업데이트
  useEffect(() => {
    setIsChanged(
      pipeMaterial !== pipe.pipeMaterial ||
        outerDiameter !== pipe.outerDiameter ||
        innerDiameter !== pipe.innerDiameter ||
        flowRate !== pipe.flowRate
    );
  }, [pipeMaterial, outerDiameter, innerDiameter, flowRate, pipe]);

  return (
    <div className="w-[400px] h-[680px] flex flex-col bg-block rounded-[30px] px-[50px] py-[30px] text-white justify-between items-center gap-5">
      <div className="flex flex-col w-full h-full">
        {/* navigate */}
        <div
          className="flex justify-end cursor-pointer hover:text-primary-200"
          onClick={onViewChange}
        >
          <p>메모</p>
          <ChevronRightIcon />
        </div>
        <div className="flex flex-col w-full h-full gap-7">
          {/* header */}
          <div className="flex flex-col items-center w-full">
            <h2 className="text-[30px] font-bold">{pipe.pipeName}</h2>
            <p className="text-[20px]">
              {pipe.pipeArea}{" "}
              {pipe.pipeFloor > 0 ? pipe.pipeFloor : `지하 ${-pipe.pipeFloor}`}
              층
            </p>
          </div>
          <div className="flex flex-col items-center w-full h-full gap-5">
            {/* 결함 탐지 */}
            <div className="flex flex-col w-full">
              <h3 className="text-[20px] font-bold self-start px-1">
                결함 확인
              </h3>
            </div>

            {/* 파이프 속성 */}
            <div className="flex flex-col w-full gap-4">
              <h3 className="text-[20px] font-bold self-start px-1">
                파이프 속성
              </h3>
              <div className="flex items-center justify-between w-full px-1">
                <div className="w-[100px] px-1">재질</div>
                <PipeMaterialListbox value={pipe.pipeMaterial} onChange={setPipeMaterial} />
              </div>
              <div className="flex items-center justify-between w-full px-1">
                <div className="w-[100px] px-1">Outer Diameter</div>
                <div className="relative w-full">
                  <Input
                    type="number"
                    value={outerDiameter}
                    onChange={(e: ChangeEvent<HTMLInputElement>) =>
                      setOuterDiameter(Number(e.target.value))
                    }
                    className={clsx(
                      "block w-full pl-5 pr-10 rounded-md border-none bg-black/40 py-2 px-3 text-sm/6 text-white",
                      "focus:outline-none data-[focus]:outline-2 data-[focus]:-outline-offset-2 data-[focus]:outline-white/25"
                    )}
                    style={{ paddingRight: "3rem" }} // 단위 공간 확보
                  />
                  <span className="absolute text-sm text-gray-800 transform -translate-y-1/2 pointer-events-none right-3 top-1/2">
                    mm
                  </span>
                </div>
              </div>
              <div className="flex items-center justify-between w-full px-1">
                <div className="w-[100px] px-1">Inner Diameter</div>
                <div className="relative w-full">
                  <Input
                    type="number"
                    value={innerDiameter}
                    onChange={(e: ChangeEvent<HTMLInputElement>) =>
                      setInnerDiameter(Number(e.target.value))
                    }
                    className={clsx(
                      "block w-full pl-5 pr-10 rounded-md border-none bg-black/40 py-2 px-3 text-sm/6 text-white",
                      "focus:outline-none data-[focus]:outline-2 data-[focus]:-outline-offset-2 data-[focus]:outline-white/25"
                    )}
                    style={{ paddingRight: "3rem" }} // 단위 공간 확보
                  />
                  <span className="absolute text-sm text-gray-800 transform -translate-y-1/2 pointer-events-none right-3 top-1/2">
                    mm
                  </span>
                </div>
              </div>
            </div>

            {/* 유체 속성 */}
            <div className="flex flex-col w-full gap-4">
              <h3 className="text-[20px] font-bold self-start px-1">
                유체 속성
              </h3>
              <div className="flex items-center justify-between w-full px-1">
                <div className="w-[100px] px-1">재질</div>
                <PipeMaterialListbox value={pipe.pipeMaterial} onChange={setPipeMaterial} />
              </div>
              <div className="flex items-center justify-between w-full px-1">
                <div className="w-[100px] px-1">Flow Rate</div>
                <div className="relative w-full">
                  <Input
                    type="number"
                    value={flowRate}
                    onChange={(e: ChangeEvent<HTMLInputElement>) =>
                      setFlowRate(Number(e.target.value))
                    }
                    className={clsx(
                      "block w-full pl-5 pr-10 rounded-md border-none bg-black/40 py-2 px-3 text-sm/6 text-white",
                      "focus:outline-none data-[focus]:outline-2 data-[focus]:-outline-offset-2 data-[focus]:outline-white/25"
                    )}
                    style={{ paddingRight: "3rem" }} // 단위 공간 확보
                  />
                  <span className="absolute text-sm text-gray-800 transform -translate-y-1/2 pointer-events-none right-3 top-1/2">
                    m<sup>3</sup>/s
                  </span>
                </div>
              </div>
            </div>
            <Button
              className={`h-fit w-fit px-5 py-2 text-white rounded-3xl ${
                isChanged
                  ? "bg-primary-500"
                  : "bg-button-background cursor-not-allowed"
              }`}
              disabled={!isChanged}
            >
              속성 변경
            </Button>
          </div>
        </div>
      </div>
    </div>
  );
};
